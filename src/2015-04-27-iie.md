...
more and I've been editing source code with it.
...
BTW if you don't use editorconfig, this is a great chance to start now.  It only
makes sense to use it.

After I became tired of having to close and open a file/buffer in order for the
config to be reloaded, or to just restart Emacs, I started playing around with
hooks.

Now the problem, I find, with hooks is that they can be a bit verbose, taking
into account I come from Vim, where filetype specific configs are simple and
short.

Another problem is that sometimes a mood doesn't offer a hook for some reason.
Those can be a bit annoying to deal with, also I had trouble finding what the
name of the hook is.

I am here to answer all of that, hopefully.
...
you're interested in, you'll be able to find them this way.
...
know why, but I figured it out trying to search for variations of "javascript".

## My indentation strategy

Now it took me a bit, but I deviced a system that works pretty nicely and is not
*that* verbose, it is a bit tedious though if you don't have [Lispy][l] or
paredit.
...
You can read about what these variables do with `C-h v`.  And what
`number-sequence` does with `C-h f`. :)
...
Feel free to name it however you want, but do add the `my-` or your username or
at the beginning so it doesn't conflict with any other variables.
...
Note the last one, you can basically alias one mode to another by using
something like what you see in the last line.
